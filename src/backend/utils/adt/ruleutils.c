...
pg_get_expr_worker(text *expr, Oid relid, int prettyFlags)
{
	...
	if (OidIsValid(relid))
	{
		rel = try_relation_open(relid, AccessShareLock, 0); /* just trying to parse
															 * node tree to convert
															 * to sql so don't care
															 * abt forking */
		if (rel == NULL)
			return NULL;
						  ...
						  deparse_columns *colinfo)
{
	...
	if (rte->rtekind == RTE_RELATION)
	{
		...
		rel = relation_open(rte->relid, AccessShareLock, 0);	/* just care abt main
																 * fork metadata */
