 ...
 * currently only need an atomic counter for the actual forkid may change in the future so treating it as if it was large size.
 * putting it here since it seems both ipci.c and dbcommands.c are aware of this file gotta figure out how to make a new .c/.h file later for this shmem
 *
 */
typedef struct
{
